...
├── REFACTOR_PLAN.md           重构蓝图与审计记录
│
├── monitors/                ★ 监控包（重构后集中管理）
│   ├── __init__.py            公共符号导出
│   ├── common.py              两个 monitor 共享逻辑（常量 / 合约加载 / ZMQ 解析 / 初始化流程）
│   ├── term_monitor.py        终端监控完整实现（rich 表格，Wind / ZMQ 双模式）
│   └── web_monitor.py         网页监控完整实现（Flask + ZMQ，暗色主题）
│
├── config/
...
### 3.2 `monitors/` — 监控包（重构核心）

重构前，`term_monitor.py` 和 `web_monitor.py` 各自独立实现了 10+ 处相同逻辑（含细节差异），现已全部提取到 `monitors/common.py`：
...
| `monitors/term_monitor.py` | 终端 UI（rich 表格渲染、Wind 行情拉取、主循环）|
| `monitors/web_monitor.py` | Flask 路由、ZMQ 消费线程、前端 HTML/JS |

根目录的 `term_monitor.py` / `web_monitor.py` 是**轻量启动入口**，仅做转发，不含业务逻辑。
...
term_monitor.py --source zmq       ← 转发至 monitors/term_monitor.py，可随时重启
    ├── 启动：读 snapshot → 恢复 TickAligner + Wind 查乘数
    ├── 运行：ZMQ SUB → 更新 TickAligner
    └── 每N秒：scan_opportunities → rich 表格（正常合约 + 分隔线 + 调整型）

web_monitor.py                     ← 转发至 monitors/web_monitor.py，可随时重启
...
# 也可通过包直接调用
python -m monitors.term_monitor --source zmq
```
...
# 也可通过包直接调用
python -m monitors.web_monitor --port 8080
```
...
`metadata/上交所期权基本信息.csv`：11,102 条记录，UTF-8-BOM 编码。
